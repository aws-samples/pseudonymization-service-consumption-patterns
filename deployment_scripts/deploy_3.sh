...
while getopts ":s:b:p:r:e:x:" opt; do
  case $opt in
...
if ((OPTIONS_FOUND !=6)); then
  echo "Please make sure to pass all the required options \"-s -b -p -r -x\""
  exit 1
fi
...
aws cloudformation deploy --template-file ${OUTPUT_TEMPLATE} \
     --parameter-overrides EPURL=${EP_URL} \
     --capabilities CAPABILITY_IAM \
     --stack-name ${STACK_NAME} \
     --region ${AWS_REGION} \

aws secretsmanager put-secret-value --secret-id EncryptionKeys-${STACK_NAME} --secret-string ${API_SECRET}
